...
to update-turtles
  ask turtles [
    ...
    set income income + random-float 2 - 1
...
to move-turtles
  ask turtles[
    ...
    ;if the turtle found a house (or didn't move) and still can't afford it (by a large margin), it will be forced to "leave the city"
    set wealth (value - income)
    if (wealth > 75 and leave?) [
      die
    ]
...
to-report Q1_avg_wealth
  report (sum [(- wealth)] of turtles-on patches with [pxcor > 25 AND pycor > 25]) / count turtles-on patches with [pxcor > 25 AND pycor > 25]
end

to-report Q2_avg_wealth
  report (sum [(- wealth)] of turtles-on patches with [pxcor <= 25 AND pycor > 25]) / count turtles-on patches with [pxcor <= 25 AND pycor > 25]
end

to-report Q3_avg_wealth
  report (sum [(- wealth)] of turtles-on patches with [pxcor <= 25 AND pycor <= 25]) / count turtles-on patches with [pxcor <= 25 AND pycor <= 25]
end

to-report Q4_avg_wealth
  report (sum [(- wealth)] of turtles-on patches with [pxcor > 25 AND pycor <= 25]) / count turtles-on patches with [pxcor > 25 AND pycor <= 25]
end
...
"" ""
PENS
"Q1 Property Values" 1.0 0 -2674135 true "" "plot Q1_avg_values"
"Q2 Property Values" 1.0 0 -14070903 true "" "plot Q2_avg_values"
"Q3 Property Values" 1.0 0 -15040220 true "" "plot Q3_avg_values"
"Q4 Property Values" 1.0 0 -955883 true "" "plot Q4_avg_values"
